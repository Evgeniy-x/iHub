...
const SpecialistsContent = ({ items }) => {
  const filterOptions = [
  ...
  switch (filterFeature.value) {
    case filterOptions[1].value:
      arrForRender = items.filter(
        (e) => e.specialty === filterOptions[1].value
      ); // When choose 'Psychologists'
      break;
    case filterOptions[2].value:
      arrForRender = items.filter(
        (e) => e.specialty === filterOptions[2].value
      ); // When choose 'Psychotherapists'
      break;
    case filterOptions[3].value:
      arrForRender = items.filter(
        (e) => e.specialty === filterOptions[3].value
      ); // When choose 'Psychiatrists'
      break;
    default:
  ...
  const renderItems = arrForRender.map(
    (
      e,
      index // map items for tender
    ) => <TabItem key={index} item={e} />
  );

  return (
    <>
      <div className="specialists__select">
        <h3 className="specialists__select--title">
      ...
      {
        renderItems.length ? (
          <ul className="specialists__content">{renderItems}</ul>
        ) : (
          <h3 className="specialists__empty">
            Unfortunately, there are no such specialists.
          </h3>
        ) // render if there are no specialists of this category
      }
    </>
  );
};
