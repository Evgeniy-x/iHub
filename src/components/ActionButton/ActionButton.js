...
import { toggleItem } from "../../redux/all-items/operations";
import PropTypes from "prop-types";

const ActionButton = ({ isFavourite, isDisfavourite, textContent, id }) => {
  ...
  const toggle = async () => {
    ...
    const newItem = {
      ...
      isDisfavourite: itemIsDisfavourite,
    }; // sompile new item for sent to store and DB
    dispatch(toggleItem(newItem)); // toggle fav in store operations
  };
  return (
    <button
      className={
        isFavourite || isDisfavourite ? "active__button" : "non-active__button"
      }
      onClick={toggle}
    >
      {textContent}
